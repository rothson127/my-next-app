                    ...
                    <div className="flex gap-1 md:gap-4 justify-center py-4 md:py-12">
                        {
                            questionData.map((question) => (
                                (currentButton === question.type) ?
                                    (
                                        <button className="px-2 md:px-8 py-1 md:py-2 text-[14.55px] md:text-[25px] font-semibold border border-black rounded-full cursor-pointer overflow-hidden whitespace-nowrap text-ellipsis bg-black text-yellow " onClick={() => handleClickButton(question.type)}>
                                            {question.type}
                                        </button>
                                    ) :
                                    (
                                        <button className="px-2 md:px-8 py-1 md:py-2 text-[14.55px] md:text-[25px] font-semibold border border-black rounded-full cursor-pointer overflow-hidden whitespace-nowrap text-ellipsis " onClick={() => handleClickButton(question.type)}>
                                            {question.type}
                                        </button>
                                    )
                            ))
